...
def recv_by_size(sock, colour):
    ...
    if size_header != b'':
        ...
        if TCP_DEBUG and size_header != b'':
            print("\n" + colour + " Recv(%s)>>>%s" % (size_header, data.decode('utf-8')[:min(100, len(data))]), end='')
...
def send_with_size(sock, bdata, colour):
    len_data = len(bdata)
    ...
    if TCP_DEBUG and len_data > 0:
        print('\n' + colour + " Sent(%s)>>>%s" % (len_data, bdata.decode('utf-8')[:min(100, len(bdata))]), end='')
